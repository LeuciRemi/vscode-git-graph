...
describe('ExtensionState', () => {
	...
	describe('isAvatarStorageAvailable', () => {
		it('Should return TRUE if the avatar storage folder existed on startup', () => {
			...
			spyOnStat.mockImplementationOnce((_: fs.PathLike, callback: (err: NodeJS.ErrnoException | null, stats: fs.Stats) => void) => callback(null, {} as fs.Stats));
		...
		it('Should return TRUE if the avatar storage folder was successfully created', () => {
			// Setup
			(jest.spyOn(fs, 'stat') as jest.Mock).mockImplementationOnce((_: fs.PathLike, callback: (err: NodeJS.ErrnoException | null, stats: fs.Stats) => void) => callback(new Error(), {} as fs.Stats));
